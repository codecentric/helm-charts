...
`keycloak.persistence.existingSecretHostKey` | The key for the database host in the existing secret (if `keycloak.persistence.deployPostgres=false` and `keycloak.persistence.existingSecret != ""`) | `""`
